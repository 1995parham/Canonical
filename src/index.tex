...
Documentation happend for this future from the begining on Confluence by the product team and we updated it
based on the implementation at the end. The code and design also documented in the code base and GitLab wiki.

I also designed the machine learning pipeline for our company. This pipeline gathers data from our NATS cluster,
moves the date into the Kafka (which is set up by us using \href{https://strimzi.io/}{Strimzi} Operator)
and then we process the data using Kafka Stream and moves them into the Spark and Cassandra for the machine learning
team. This project required a very close coorporation between us and the machine learning team to test and make
sure we have all required data. We needed to make sure we don't loss any data here and for that we used blackbox monitoring
